...
int main()
{
  ...
  for (int i = 0; i < 12; i++)
  {
    ...
    // only one %d is needed, using for-loop so we can put into the array one by one

    // Below is to find the maximum grade
    if (grades[i] > max)
    {
      max = grades[i];
    }
  ...
  fclose(fileptr4);
  // Close the file after reading since printing would only need the grades[] array
  ...
  int grades_2d[4][3];             // Declare a 2D array to hold 4 rows and 3 columns of grades
  for (int r = 0; r < 4; r++)
  {
    ...
    for (int c = 0; c < 3; c++)
    {
      ...
      fscanf(fileptr4, "%d", &grades_2d[r][c]); // Text file structures doesn't matter
